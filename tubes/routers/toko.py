...
#machine learning aspects
#prediksi penjualan barang elektronik berdasarkan input biaya dari user
@router.post("/predict by input")
def predict_sales_from_input(request:schemas.atribut,current_user:schemas.User=Depends(oauth2.get_current_user)):
            tv=request.TV
            ig=request.Instagram
            newspaper=request.Newspaper
            features= np.array([[tv,ig,newspaper]])
            prediction=int(model.predict(features))
            return {
                "Prediction based on input " : prediction
                }
